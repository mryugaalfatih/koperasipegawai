        ...
        <div className="overflow-x-auto rounded-xl border border-[#dbe5f1]">
          <table className="w-full text-left text-xs">
            <thead className="bg-[#f8fbff] text-[#475569] border-b border-[#dbe5f1]">
              <tr>
                <th className="px-2 py-2 font-bold">No. Faktur POS</th>
                <th className="px-2 py-2 font-bold">Tanggal</th>
                <th className="px-2 py-2 font-bold">Pembeli / Anggota</th>
                <th className="px-2 py-2 font-bold">Metode Bayar</th>
                <th className="px-2 py-2 font-bold text-right">Grand Total</th>
              ...
              {filteredSales.length ? (
                filteredSales.map((sale) => (
                  <tr key={sale.id} className="hover:bg-[#f8fbff] transition-colors">
                    <td className="px-2 py-2 font-bold text-[#2563eb]">{sale.invoice_no}</td>
                    <td className="px-2 py-2 font-semibold text-[#64748b]">{sale.sale_date}</td>
          ...
          <div className="space-y-4 text-xs">
            ...
            <div className="rounded-xl border border-[#dbe5f1] overflow-hidden">
              <table className="w-full text-left text-[11px]">
                <thead className="bg-[#f8fbff] text-[#475569] border-b border-[#dbe5f1]">
                  <tr>
                    <th className="px-2 py-2 font-bold">Barang</th>
                    <th className="px-2 py-2 font-bold text-center">Qty</th>
                    <th className="px-2 py-2 font-bold text-right">Harga</th>
                ...
                <tbody className="divide-y divide-[#e2e8f0]">
                  {selectedSale.toko_sale_items?.map((item, idx) => (
                    <tr key={idx}>
                      ...
                      <td className="px-2 py-2 text-right">{formatRupiah(item.sell_price)}</td>
